...
$(document).ready(() => {
  ...
  formularioCompra.on('submit', (event) => {
    ...
    if (tieneTrabajo && esMayorDeEdad && modelosDisponibles.includes(auto.toLowerCase())) {
      ...
      resultadoDiv.html(!esMayorDeEdad
        ? 'Lo siento, debes ser mayor de edad para comprar un auto.'
        : tieneTrabajo
          ? `Lo siento, ${nombre} ${apellido}, debes tener trabajo para comprar un auto.`
          : 'Lo siento, no cumples con los requisitos para comprar un auto.');
    }
  });
