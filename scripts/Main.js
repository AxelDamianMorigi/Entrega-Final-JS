...
async function cargarDatosAutos() {
  try {
    const response = await fetch('scripts/autos.json');
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    autosInfo = await response.json();
  } catch (error) {
    console.error('Error al cargar los datos de los autos:', error);
  }
}
  ...
  $('#mostrarLocal').on('click', () => {
    ...
    if (autosComprados && autosComprados.length > 0) {
      ...
      for (const auto of autosComprados) {
        ...
        if (auto.cuotas === 0) {
          precio = autosInfo[auto.modelo.toLowerCase()].precioBase;
        } else {
          const autoSeleccionado = new Auto(auto.modelo, autosInfo[auto.modelo.toLowerCase()].precioBase);
